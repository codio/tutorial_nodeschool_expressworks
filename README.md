...
- Hands Dirty: open up a Terminal with from the 'Tools->Terminal' menu, then enter `npm install -g expressworks@latest`.
- Lazy Way: click on the "Install ExpressWorks" menu item. Once installed, you can close that tab.
...
You can open a Terminal window at any time by selecting the 'Tools->Terminal' menu item. Your terminal defaults to the `~/workspace` folder, which equates to the root of your Codio code project. If you are writing your code in the lesson folder (for example `02_jade`) then you will need to `cd` into that folder, so something like this
